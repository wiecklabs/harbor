...
module Harbor::ViewContext::Helpers::Cache
  ...
  #   Time.now + TTL is equal to or greater than the cache-insertion-time + maximum_age.
  ##
  def background_cache(key, ttl = 30 * 60, max_age = nil, &generator)
    store = @cache_store || Harbor::View.cache
    ...
    content = if item = store.get(key, true)
      begin
        # Continue caching the data in a background process
        if !item.fresh? && !store.locked?(key)
          store.lock(key)

          fork do
            begin
              puts "Background caching 1"
              $0 = "cache #{key} #{$0}"
              puts "Background caching 2"
              data = capture(&generator)
              puts "Background caching 3"
              store.put(key, data, ttl, max_age)
              puts "Background caching 4"
            ensure
              puts "Background caching 5"
              store.unlock(key)
            end
          end
        end
