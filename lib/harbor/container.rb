  ...
  class Container
    ...
    def register(name, service, &setup)
      ...
      @services.values.each do |service_registration|
        if service_registration.service.is_a?(Class) && service_registration.service.instance_methods.map(&:to_s).include?("#{name}=")
          dependencies(service_registration.name) << name
        end

        if type_methods.map(&:to_s).include?("#{service_registration.name}=")
