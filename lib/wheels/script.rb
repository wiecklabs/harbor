...
#   end
#   
#   Wheels::Script::Runner.new(ARGV, services, Processor).run!
##
module Wheels
  class Script
    ...
    class Runner
      ...
      def initialize(argv, services, script)
        raise ArgumentError.new("services must be a Wheels::Container but was #{services.inspect}") unless services.is_a?(Wheels::Container)
      ...
      def restart
        Wheels::Script.restart(@script.pid_file)
      end

      def stop
        Wheels::Script.kill(@script.pid_file)
      end
    end
